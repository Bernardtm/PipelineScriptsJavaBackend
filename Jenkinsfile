    ...
    node('maven'){
        ...
        openshift.withCluster() {
            openshift.withProject('maven-backend-qa') {
                ...
                stage('Deploy QA') {
                    openshift.apply(openshift.process(readFile(file:'template-maven.yml'), "-p=ENVIRONMENT=qa", "--param-file=jenkins.properties"))
            ...
            openshift.withProject('maven-backend-hml') {
                stage('Deploy HML') {
                    def template = openshift.process(readFile(file:'template-maven.yml'), "-p=ENVIRONMENT=hml", "--param-file=jenkins.properties")
